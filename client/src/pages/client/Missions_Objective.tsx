...
export default function MissionObjectives() {
  ...
  return (
    <div
      ref={sectionRef}
      className="w-full flex justify-center items-start  bg-[#ebe9e3] mt-30"
      style={{ fontFamily: "'DM Sans', sans-serif" }}
    >
      <div className="w-full max-w-[100rem] grid grid-cols-1 sm:grid-cols-[40%_60%] border-y border-black/10">

        {/* ── Left Panel ── */}
        <div
          ref={leftPanelRef}
          className="relative border-b sm:border-b-0 sm:border-x border-black/10  overflow-hidden "
        >
          ...
          >
            <h2
              className="text-[clamp(2rem,4vw,3.2rem)] font-normal leading-[1.15] text-[#1a1a18]  m-0 playfairDisplayDiv max-w-[450px] "
            >
              Our mission
              <br />
              <em className="text-black/70" style={{ fontStyle: "italic" }}>
                &amp; objectives
              </em>
            </h2>
            <p className="mt-8 text-sm leading-[1.75] text-black/50 max-w-[450px]">
        ...
        <div className="flex flex-col divide-y md:border-r md:border-black/10 md:divide-black/10">
          {OBJECTIVES.map((obj) => (
            <div
              key={obj.num}
            ...
            >
              ...
              {/* Text */}
              <div>
                <p className="text-[10px] tracking-[0.1em] uppercase text-black/30 mb-1.5">
                  {obj.num}
                </p>
                <h3
                  className="text-[18px] font-medium text-[#1a1a18] mb-2 leading-tight"
                  style={{ fontFamily: "'Playfair Display', serif" }}
                >
                  {obj.title}
                </h3>
                <ul className="text-[13px] leading-[1.7] text-black/50 m-0 list-disc pl-5 space-y-1">
                  {obj.desc.map((item) => (
                    <li key={item}>{item}</li>
                  ))}
                </ul>
