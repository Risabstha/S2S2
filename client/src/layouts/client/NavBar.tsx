  ...
  return (
    <a href="/" className="flex items-center gap-0 no-underline">
      ...
      <div className="flex items-baseline gap-0 ml-1">
        <span
          className={`font-bold text-2xl text-[#2F5064] tracking-[0.12em] select-nonej playfairDisplayDiv ${scrolled ? "" : "md:text-gray-100 text-black"}`}
        >
          Slippers
        </span>
        <span
          className={`font-bold text-2xl text-[#C18374] tracking-[0.12em] select-none playfairDisplayDiv ${scrolled ? "" : "md:text-gray-300 text-black"}`}
        >
          2
        </span>
        <span
          className={`font-bold text-2xl text-[#2F5064] tracking-[0.12em] select-none ${scrolled ? "" : "md:text-gray-100 text-black"}`}
        >
          Sat-
        </span>
      </div>
      <span
        className={` text-2xl font-bold  text-[#C18374] tracking-[0.12em] select-none playfairDisplayDiv ${scrolled ? "" : "md:text-gray-300 text-black"}`}
      >
...
function DesktopNav({ showOpaque }: { showOpaque: boolean }) {
  ...
  return (
    <div
      className={`hidden lg:flex items-center justify-between w-full max-w-400 mx-auto px-8 h-22.5 `}
    >
        ...
        {NAV_ITEMS.map((item) => (
          ...
          >
            {item.sub.length > 0 ? (
              ...
              <a
                href={item.href}
                className={`flex items-center gap-1  text-[20px]  px-3 py-7.5 rounded-lg transition-all duration-100 cursor-pointer no-underline ${showOpaque ? " hover:text-gray-500" : "text-white hover:text-gray-300"}`}
              >
                {item.label}
              </a>
            )}
                ...
                {item.sub.map((s) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0 ">
                      ...
                      <p className="text-[15px] mt-0.5 leading-relaxed">
                        {s.desc}
                      </p>
                    </div>
                  </a>
...
function MobileNav({ showOpaque }: { showOpaque: boolean }) {
  ...
  return (
    <>
      {/* Mobile top bar */}
      <div className="flex lg:hidden items-center justify-between px-5  h-15.5">
        <Logo scrolled={showOpaque} />
        <button
      ...
      {/* Mobile drawer */}
      {menuOpen && (
        <div className="fixed inset-0 z-50 lg:hidden bg-[#ebe9e3]">
          <div
                  ...
                  {item.sub.length > 0 ? (
                    <>
                      <div className="w-full flex ">
                        <a
                          href={item.href ?? "#"}
                          className="w-[80%] flex items-center justify-between py-3.5 text-[18px] rounded-lg px-3 hover:bg-black/5  font-medium text-[#2F5064] hover:text-[#183148] transition-colors no-underline"
                        >
                          {item.label}
                        </a>
                        <button
                          onClick={() => toggleItem(item.label)}
                          className="w-[20%] flex items-center justify-center py-3.5 text-[18px] rounded-lg px-3 hover:bg-black/5  font-medium text-[#2F5064] hover:text-[#183148] transition-colors no-underline"
                        >
                        ...
                        <div className="pb-2 pl-2 flex flex-col gap-1">
                          {item.sub.map((s) => (
                            ...
                            >
                              ...
                              <div>
                                <div className="flex items-center gap-1.5">
                                  <span className="text-[13px] font-medium text-[#2F5064]">
                                    {s.label}
                                  </span>
                                  {s.external && <ExternalArrow />}
                                </div>
                                <p className="text-[11.5px] text-[#2F5064] mt-0.5 leading-relaxed">
                                  {s.desc}
                                </p>
                              </div>
                            </a>
                    ...
                    <a
                      href={item.href ?? "#"}
                      className="flex items-center py-3.5 text-[18px] hover:bg-black/5 px-3 rounded-lg font-medium text-[#2F5064] hover:text-[#183148] transition-colors no-underline"
                    >
                      {item.label}
                    </a>
                  )}
